...
Buffers the jobs pushed by AE2 crafting CPUs inside nine private storage-cell slots and executes them in batches once no new material has arrived for a short while (two delay modes: standard 40 ticks, fast 10 ticks; the buffer itself has no size limit). Supports crafting-table, smithing-table and stonecutting recipes. If the network cannot take a batch's outputs, the consumed inputs roll back into the cells and the run is retried later. Storage cells inserted here are private to the machine (never exposed to the ME network) and are locked while work is buffered; breaking the block or pressing cancel returns the buffer to the network. Up to **four AE2 Speed Cards** add worker threads (1 by default, x2 each, up to 16 threads): a single-pattern order stays single-threaded, while chained orders drain their queued patterns in parallel. Outputs are returned to the network smoothly (~5% of the accumulated total per tick over ~20 ticks) so huge orders never hitch the server with massive single-tick storage IO. Total output always matches the ordered amount.
