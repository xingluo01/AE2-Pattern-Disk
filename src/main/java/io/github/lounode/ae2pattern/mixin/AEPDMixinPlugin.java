...
public final class AEPDMixinPlugin implements IMixinConfigPlugin {
    ...
    private static @Nullable Boolean decideEco() {
        ...
        if (!contractPresent()) {
            LOGGER.info("NEO ECO is present without a usable parallel intake contract, keeping the one-craft push path");
            return Boolean.FALSE;
        }
    ...
    /**
     * Whether the contract still looks like the one this mixin implements: exactly one abstract method - the
     * slot count the bridge answers - plus the batch entry point with the signature that gets forwarded. A
     * contract that grew another abstract method would otherwise fail much later, inside ECO's dispatch, as an
    ...
    private static boolean contractPresent() {
        ...
        long abstractMethods = Arrays.stream(contract.getMethods())
                .filter(method -> Modifier.isAbstract(method.getModifiers()))
                .count();
        if (abstractMethods != 1) {
            LOGGER.info("NEO ECO parallel intake disabled: {} has {} abstract methods, expected 1",
                    ECO_PARALLEL_PROVIDER, abstractMethods);
            return false;
        }
        try {
            contract.getMethod("eco$getAvailableParallelSlots");
            contract.getMethod("eco$pushPatternBatch", IPatternDetails.class, KeyCounter[].class, long.class, UUID.class);
        } catch (NoSuchMethodException changed) {
            LOGGER.info("NEO ECO parallel intake disabled: {} no longer carries the expected entry points",
                    ECO_PARALLEL_PROVIDER);
            return false;
        }
        return true;
    }

    /** The contract class, or {@code null} when no class loader of this environment can see it. */
    private static @Nullable Class<?> loadContract() {
        ClassLoader[] loaders = {
                AEPDMixinPlugin.class.getClassLoader(),
        ...
        for (var loader : loaders) {
            ...
            try {
                return Class.forName(ECO_PARALLEL_PROVIDER, false, loader);
