 ...
 * 被加载——把接口写进父类签名，没装 EAE+ 的玩家一开终端就 {@code NoClassDefFoundError}。父类添加别的
 * EAE+ 接口时同样照此办理，不要直接写进父类的 implements。</p>
 *
 * <p>{@link ExtendedAEPlusCompat#createUploadMenu} 用反射按类名实例化本类（父类的字节码里不能出现本类，
 * 否则加载父类就会连带解析本类 implements 的接口），因此**本类与它的构造器必须保持 public**，否则那条
 * 反射入口取不到构造器。</p>
 */
